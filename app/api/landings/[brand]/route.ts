...
function normalizeLanding(
  ...
  return {
    ...
    form: {
      ...
      campus: normalizedBody.form?.campus || "",
      campusOptions: (normalizedBody.form?.campusOptions ?? []).map((option) => ({
        label: option?.label || "",
        campus: option?.campus || "",
        campaigntype: option?.campaigntype || "",
      })),
      language:
        normalizedBody.form?.language ||
        (language === "es" ? "Spanish" : "English"),
      campaigntype: normalizedBody.form?.campaigntype || "",
      campaigncode: normalizedBody.form?.campaigncode || "",
      leadsource: normalizedBody.form?.leadsource || "",
      leadid: normalizedBody.form?.leadid || "",
      tenantid: normalizedBody.form?.tenantid || "",
      schoolname: normalizedBody.form?.schoolname || "",
      channel: normalizedBody.form?.channel || "",
      veritySysKey: normalizedBody.form?.veritySysKey || "",
      verityLeadPostUrl: normalizedBody.form?.verityLeadPostUrl || "",
      hiddenProgramFieldName:
        normalizedBody.form?.hiddenProgramFieldName || "program",
